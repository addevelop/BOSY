...
class getOrder
{
    ...
    {
        $getOrdersById = false;
        $stmt = $this->db->prepare("SELECT orders.ID_order, orders.numero as 'numero', orders.created_at, orders.ID_user, SUM(basket_order.quantity) as 'quantity', SUM(basket_order.price * basket_order.quantity) as 'total', images.path, status_orders.title as 'status_orders' FROM orders INNER JOIN basket_order ON orders.numero = basket_order.numero INNER JOIN images ON basket_order.ID_product = images.ID_product INNER JOIN status_orders ON orders.status = status_orders.status WHERE orders.ID_user = :iduser GROUP BY orders.numero");
        $stmt->bindParam(":iduser", $this->iduser, PDO::PARAM_INT);
        if($stmt->execute())
        {
    ...
    {
        $getOrderByNum = false;
        $stmt = $this->db->prepare("SELECT orders.ID_order, orders.numero as 'numero', orders.created_at, basket_order.title, orders.ID_user, images.path, status_orders.title as 'status_orders', basket_order.quantity, basket_order.price as 'price' FROM orders INNER JOIN basket_order ON orders.numero = basket_order.numero INNER JOIN images ON basket_order.ID_product = images.ID_product INNER JOIN status_orders ON orders.status = status_orders.status WHERE orders.ID_user = :iduser AND orders.numero = :numorder");
        $stmt->bindParam(":iduser", $this->iduser, PDO::PARAM_INT);
